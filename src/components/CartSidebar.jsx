    ...
    <>
      ...
      <aside className={`cart-sidebar ${isCartOpen ? 'open' : ''}`}>
        <div className="flex items-center justify-between p-6 border-b border-border">
          <h2 className="font-serif text-2xl font-medium text-text-primary">Your Cart</h2>
          <button
        ...
        {cart.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
            <svg className="text-text-muted mb-6 opacity-50" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
              <path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"></path>
              <line x1="3" y1="6" x2="21" y2="6"></line>
              <path d="M16 10a4 4 0 0 1-8 0"></path>
            </svg>
            <p className="font-serif text-xl text-text-primary mb-2">Your cart is empty</p>
            <span className="text-sm text-text-muted">Add some haunting pieces to get started</span>
          </div>
        ) : (
          <>
              ...
              {cart.map(item => (
                <div key={item.id} className="flex gap-4 py-4 border-b border-border last:border-b-0">
                  <div
                    ...
                    style={{ '--hue': item.hue }}
                  >
                    <span className="font-serif text-2xl text-white/20">{item.name.charAt(0)}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2 border border-border rounded">
                        ...
                        <span className="text-sm text-text-primary min-w-[1.5rem] text-center">{item.quantity}</span>
                        <button
                          className="bg-transparent border-none text-text-secondary py-1 px-2 cursor-pointer text-base hover:text-text-primary transition-colors duration-300"
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
                        >
            ...
            <div className="p-6 border-t border-border bg-bg-secondary">
              <div className="flex justify-between mb-2">
                ...
                <span className="font-mono text-xl font-bold text-text-primary">${cartTotal.toFixed(2)}</span>
              </div>
              <p className="text-xs text-text-muted mb-6">Shipping calculated at checkout</p>
